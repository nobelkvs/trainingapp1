...
$("#DeleteCompanyForm").submit(function (event) {
    ...
    function successM(data) {
        if(data >= 1){
            var timeOut = 2;
            $('#DeleteMessage').addClass("success").html("successfully Deleted").fadeIn;
            setTimeout(function(){
                jQuery('#DeleteMessage').fadeOut()
             jQuery('#DeleteMessage').css("display", "none")
            }, timeOut * 1000);
        }
        else
        $('#DeleteMessage').addClass("Failure").html("No Such Record Found");
    }
...
$("#deleteShow").click(function () {
    $("#DeleteCompanyForm").css("display", "block");
    $("#CompanyForm").css("display", "none");
    $("#RetrieveCompanyForm").css("display", "none");
    $("#HomeForm").css("display", "none");
