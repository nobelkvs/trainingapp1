...
public class Controller extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        //Getting the action parameter from the UI
        String action = req.getParameter("action");
        ControllerImpl controllerImplObj = new ControllerImpl();
        //By using the action parameter calling the appropriate controller implementation methods.
        if (action.equals("create")) {
            int status = controllerImplObj.createPolicy(req, res);
            if(status > 0){
                out.println("Policy is created successfully your policy Id is : " + status);
            }else if(status == -1){
                out.println("Mail Id already exists enter another mail Id");
            }else {
            	out.println("An error occured with the connection please try again");
            }
            ...
            if(status > 0){
            	...
            	out.println("entered Id is not found");
            }else{
