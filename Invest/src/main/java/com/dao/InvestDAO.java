...
public class InvestDAO implements InvestDAOint {
    ...
    private static final String SQL_GET_ADMIN = "SELECT pswd FROM Admin where uname=?";
    /**
    ...
    public int checkAdmin(admin_model admin) {
        Connection conn = null;
        int insertStatus = 0;
        try {
            conn = c.getConnect();
        ...
        String given_pswd=admin.getPassword();
        log.info(given_pswd);
        try {

            conn.setAutoCommit(false);
            ps = conn.prepareStatement(SQL_GET_ADMIN);
            ...
            if(password.equalsIgnoreCase(given_pswd))
            {
                return 1;
            }
            else
