...
function displayCreateTaskForm() {
	...
	$("#createTaskForm").attr("style",
			"margin: 0 auto;width: 80%;display:block");// now displaying the Task Creation Form
...
function displayCreated(data) { // displaying the return message(either task created or not) from the server
	...
	if (data.includes("Duplicate")) {
		$("#message").html("This Task Already Created");
...
function displayRetrieveTaskForm() { // Displaying the Search Form to retrieve the data
	...
	$("#searchForm").submit(function(event) { //on submitting the form retrieving the data from the fields
		...
		var searchString = document.getElementById("searchFieldValue").value;//retrieving the searching string
...
function displayRetrieved(data) {
	...
	if (!jQuery.isEmptyObject(data)) {
		...
		$(data)
				.each(
						function(index, value) {
							...
							cell1.innerHTML = "<input name='selector[]' type='checkbox' value='"
									+ value.taskId + "'>";
		...
		$("#deleteButton").attr("style",
				"margin-left:6.5cm;margin-top:2cm;display:block");
		...
		$("#retrieveTable").attr("style", "display:none");
		$("#deleteButton").attr("style", "display:none");
		$("#deleteMessage").attr("style",
				"color:red;font-size: 1cm; display: block")
...
function deleteStatus(msg) {
	...
	$("#deleteMessage").attr("style",
			"color:red;font-size: 2cm; display: block")
